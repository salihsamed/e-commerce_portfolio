...
const CartComp = ({cart}) => {
  ...
  return (
    ...
    <div className='w-1/12 max-sm:w-2/12 flex items-center gap-1 justify-center'><button  className={`rounded-lg border-gray-200 border-2 w-5 h-5 text-xl bg-gray-200 flex justify-center items-center ${cart?.quantity===1?" ":"hover:bg-gray-300"} `}  onClick={()=>dispatch(changeQuantity({id:cart?.id,type:"dec"}))} disabled={cart?.quantity===1}><AiOutlineMinus/></button>&nbsp;{cart?.quantity}&nbsp;<button  className='rounded-lg border-gray-200 border-2 w-5 h-5 text-xl bg-gray-200 flex justify-center items-center hover:bg-gray-300' onClick={()=>dispatch(changeQuantity({id:cart?.id,type:"inc"}))}><AiOutlinePlus/></button></div>
